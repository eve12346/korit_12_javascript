...
/**
 * 실무 사용 사례
 * 전화번호 입력 받을 때 - 없어야 하면 걸러내야하기 때문에 '010-1234-5678'이라는 string 값(number라면 010이 불가능합니다)에 .ㅑindexOf('-')를 확인하며 -1이 return되면 다음 단계로, 3번지가 추렭된다면 '-'을 제거하는 메서드로 넘길 수 있습니다
 */
...
console.log(strFruits[-1]);
// python 에서는 마이너스인덱스가 있기는 한데, JS에서는 없습니다. 하지만 slice()의 argument로 마이너스값을 넣어줄 수는 있습니다.
console.log(strFruits.slice(-12));  // 결과값 Banana, Kiwi
/*
시작점이 -12qjswlrh rm ekdmadl -11 -10 ... -1(끝)까지 추출하는 메서드
근데 -1하니까 i가 짤리는데 이거 좀 나중에 처리하겠습니다.
 */
console.log(strFruits.substring(7 , 13));   // 결과값 : Banana

/*
    그러면 substirng() / slice() 가 동일한 기능을하는 것 같아 보이는데, substring()은 음수값을 허용하지 않습니다.
*/
...
// SEOUL은 찾고자하는 문자열 I는 insensitive의 약자로 덜까다롭게 굴겠다는 것입니다.
