...
## Object Loteral Synrax Extension
- JS에서의 Object는 key-value propertyㄹ 이루어져있는데 기본적으로 key는 string자료형.
...
1. LocalStorage
  - 저장된 데이터를 삭제하기 전까지 영구히 보존됨. 저장되는 모든 데이터는 JS 객체처럼 key - value pair로 이루어져있습니다. 단, 저장되는 모든 데이터 값은 string으로만 저장가능합니다. 그래서 이 저장소에 number, object, boolean, array 등과 같은 string이 아닌 데이터를 저장할 때는 JSON. stringify를 통해 문자열로 변환하여 저장해야 합니다. 그리고 localStorage의 데이터를 JS로 읽고 싶다면 JSON.parse()를 사용하여 객체로 변환해줘야 연산이 가능합니다.
    -  로컬스토리지는 모단에 위배되지 않고, 영구히 저장해도 상관없는 데이터를 저장할 때 적합..
...
console.log(kim);
```
- 이상의 코드의 주의 사항으로는 localStorage를 참조해야 하기 때문에 html파일도 요구됩니다. live server에서 확인할 것

2. SessionStorage
- localStorage는 지우기 전까지 영구 보존됩니다.
- sessionStorage는 새로고침하면 다 날라갑니다.

todolist 작성한 부분을 github에 올릴건데 중요한 부분은 ai를 사용하고 나서 어떻게 뭍여넣을지 입니다.

- todolist.html을 기준으로 보냈기 때문에 0.css와 .js가 기존 버전으로 도외어있어서 html만 BEM버전으로 바꾼다고 하더라도 실제로는 CSS가 적용되지 않습니다.

- 특정 방법을 사용했을 때 여러분이 작성한거 다 날리고 덮어쓸 수도 있겠지만 남겨놓고 CSS버전만 다양하게 갈아끼우고 싶다면 남겨두는 방법도 필요합니다

- 레퍼런스를 암 ㅏAI로 만들 확률이 높기 때문에 기존 문서에서 적용하기 위해서는 link 태그 사용 후에 href를 잘 적용시켜야 합니다.
